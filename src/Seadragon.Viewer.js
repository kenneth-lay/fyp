...
(function() {
    ...
    SeadragonViewer = Seadragon.Viewer = function(container) {
        ...
        function onCanvasClick(tracker, position, quick, shift) {
            if (!shrinked && viewport && quick && !panoramaNavigatorEnter && !annotationMouse && !duringActive && !postExitAnno2 && !checkPolygonsLock && !inZoomBar) {    // ignore clicks where mouse moved	
        ...
        function onCanvasPress(tracker, position) {
            if (!shrinked && viewport && !annotationMouse) {
        ...
        function onCanvasDrag(tracker, position, delta, shift) {
            if (!shrinked && viewport && !panoramaNavigatorEnter && !annotationMouse && !duringActive && !postExitAnno2 && !checkPolygonsLock) {
        ...
        function onCanvasScroll(tracker, position, delta, shift) {
            if (!shrinked && viewport && !panoramaNavigatorEnter && !annotationMouse && !duringActive && !postExitAnno2 && !checkPolygonsLock) {
                ...
                if (viewport.getZoom() >= 0.922 && viewport.getZoom() * factor >= 0.922)
                {
                    viewport.zoomBy(factor, viewport.pointFromPixel(position, true));
                    viewport.applyConstraints();   
                }
                ...
                {
                	viewport.zoomTo(1, viewport.pointFromPixel(position, true));
                    viewport.applyConstraints();   
                }
				UpdateZoomBar(1);
				if (!annoFadeOutLock)
